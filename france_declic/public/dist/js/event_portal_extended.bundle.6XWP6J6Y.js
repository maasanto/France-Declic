(() => {
  // ../france_declic/france_declic/public/js/event_portal_extended.bundle.js
  frappe.events.EventsPortalView = class EventsPortalView extends frappe.ui.BaseWebCalendar {
    constructor(opts) {
      var _a;
      (_a = opts.wrapper) != null ? _a : opts.wrapper = opts.parent;
      super(opts);
    }
    calendar_options() {
    ...
    showEnhancedModal(event, slots) {
      ...
      dialog.fields_dict.event_content.$wrapper.html(this.renderEnhancedEventContent(event, slots));
    ...
    handleSlotSignup(slotName, slotTitle, button, eventId, slots) {
      frappe.confirm(__("Are you sure you want to sign up for '{0}'?", [slotTitle]), () => {
        ...
        frappe.call({
          ...
          callback: (r) => {
            if (r.message || !r.exc) {
              ...
              button.removeClass("btn-primary").addClass("btn-success").html('<i class="fa fa-check"></i> ' + __("Signed Up"));
