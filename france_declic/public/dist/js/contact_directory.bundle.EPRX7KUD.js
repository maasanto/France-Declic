(() => {
  // ../france_declic/france_declic/public/js/contact_directory.bundle.js
  var ContactDirectory = class {
    constructor(wrapper) {
      this.ready = false;
    ...
    async show_contacts() {
      ...
      try {
        ...
        const contacts = ((_a = response.message) == null ? void 0 : _a.contacts) || [];
    ...
    show_no_results() {
      var _a, _b, _c, _d;
      const message = ((_a = this.filter_values) == null ? void 0 : _a.search) || ((_b = this.filter_values) == null ? void 0 : _b.workshop_type) ? __("No contacts found matching your criteria") : __("No contacts found");
      this.contacts_section.innerHTML = `
			<div class="no-results">
				${frappe.utils.icon("users")}
				<div class="mt-2">${message}</div>
				${((_c = this.filter_values) == null ? void 0 : _c.search) || ((_d = this.filter_values) == null ? void 0 : _d.workshop_type) ? `<button class="btn btn-sm btn-secondary mt-2" onclick="contact_directory.clear_filters()">
						${__("Clear Filters")}
					</button>` : ""}
			</div>
		`;
    ...
    render_contact_card(contact) {
      ...
      if (contact.custom_city)
        location_parts.push(contact.custom_city);
      if (contact.custom_pincode)
        location_parts.push(contact.custom_pincode);
    ...
    clear_filters() {
      ...
      if (this.workshop_filter)
        this.workshop_filter.set_value("");
      this.refresh_search(true);
    }
  ...
  var ContactDirectorySidebar = class {
    constructor(wrapper) {
      this.wrapper = wrapper;
      this.get_stats().then(() => {
        this.build();
